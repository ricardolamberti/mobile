...
import 'JSONCheckboxField.dart';
import 'JSONDiv.dart';
import 'JSONIcon.dart';
import 'JSONSelectField.dart';
...
class DataPopUp extends StatelessWidget {
  const DataPopUp(this.popup,this.onPressed,this.context);
  ...
  Widget addMenuItem(AstorComponente root) {
    return TextButton(
        onPressed: () { onPressed(root,context);},
        child: ListTile(
  ...
  Widget _buildTiles(AstorComponente root) {
    ...
    if (root.widget==WidgetType.li) return addMenuItem(root.components.first);
    if (root.type==WidgetType.li) return addMenuItem(root.components.first);;
